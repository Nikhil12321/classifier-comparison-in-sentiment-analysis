...
def maxE(file_name):
	...
	def getFeatureVector(review):
	    ...
	    for w in words:
	        ...
	        if (w in stopWords or val is None):
	            ...
	            featureVectorr.append(stemmer.stem(w.lower()))
	...
	fp = csv.reader(open(file_name, 'r'), delimiter=',')
	reviews = []
	testing_set = []
	featureList = []
	count = 1
	...
	for row in fp:
	    ...
	    if(count >= 945 and count <= 1000):
	        testing_set.append((featureVector, sentiment))
	    elif(count >= 1945 and count <= 2000):
	        testing_set.append((featureVector, sentiment))
	    elif(count >= 501 and count <= 1000):
	    	print "useless"
	    elif(count >= 1501 and count <= 2000):
	    	print "useless"
	    else:
	        featureList.extend(featureVector)
	        reviews.append((featureVector, sentiment))

	    print "training %d for %s"%(count, file_name)
	    count = count + 1
	...
	maxEnt = nltk.classify.maxent.MaxentClassifier.train(training_set, 'GIS', trace=3, \
	                    encoding=None, labels=None, gaussian_prior_sigma=0, max_iter = 1)
	...
	for test in testing_set:
	    result = maxEnt.classify(extract_features(test[0]))
